    ...
    public class VideosController : Controller
    {
        ...
        public ActionResult Create(Video video)
        {
            ...
            for (int i = 0; i < Request.Files.Count; i++)
            {
                ...
                if (Request.Files.GetKey(i) == "upload")
                {
                    pathToSave = Server.MapPath("~/Content/Videos/");
                    video.videoPath = Path.Combine(pathToSave, Request.Files[i].FileName);
                }
                else if (Request.Files.GetKey(i) == "uploadPowerPoint")
                {
